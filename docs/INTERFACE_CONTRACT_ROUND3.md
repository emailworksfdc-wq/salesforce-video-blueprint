...
| ID | File | Measured evidence | Root cause |
|----|------|-------------------|------------|
| **Router-action overflow** | `naming.py` | `naming.MAX_NAME_LENGTH` was 80 and capped topic/subagent names, but `router_action_name` returns `go_to_<subagent>`, 6 chars longer. Measured: an 80-char intent produced an **86-char router action**, and `validate_locally` reported **0 findings**, so it would reach an org unchecked. | The length cap was set to the API limit, but derived names add prefixes. The overflow was silent because validation only checked the base names, not the prefixed outputs. Fixed by reducing the cap to **74** to budget the prefix. |
