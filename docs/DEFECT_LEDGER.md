...
| **D9** | G5 audit | MEDIUM | Offline refinement is decorative on real derived input | `run_pipeline('examples/case_creation_aft3.dom_capture.jsonl')` yields a spec with 143 unique orchestration steps and 3 unique guardrails (57×`input on text:lightning-textarea#N`, 57×`records-record-layout-text-area#N`, etc.). `refine(spec, use_cli=False, max_rounds=5)` ran 3 rounds, `stop_reason="Converged: improvement < 2 for 2 consecutive rounds"`, score = 85/100 on v1, v2, v3 — **no lever ever fired**, every round's notes read `"No offline improvements applied…"`. The only blocking issue is `telemetry_source='mock'`, which no offline text mutation can cure. The dedupe lever only matches EXACT string equality; real captures have monotone-numbered near-duplicates (`#5` vs `#6`) that string-equality misses. Perturbing the spec with EXACT duplicates triggers the lever (score 83 → 85, +2), confirming the levers work but never fire on unperturbed real input | `_apply_offline_improvements` levers (vague-word strip, camelCase, exact-string dedupe, `validate input` rewrite) match patterns `spec_builder` does not emit or match substrings the real capture shape does not produce. G5 ("refinement is EFFECTIVE") was proven only against constructed fixtures, never end-to-end on `build_agent_spec(real_capture)` output | `iterate.py:561+` (`_apply_offline_improvements`) | Reported, not fixed | `test_refine_effective_on_real_derived_input` (added; PASSES because it applies a perturbation the levers CAN cure, but the underlying decorative-on-real-input problem remains and warrants either (a) a template-based dedupe lever that canonicalises `#\d+` runs before comparison, or (b) documenting that offline refinement is not expected to move the score on well-formed captures) |
